...
export class ShoppingListComponent implements OnInit {
  ...
  private connectSignalR(): void {
    this.signalrService.connect().then(() => {
      this.signalrService
        .getHubConnection()
        .on('ReceiveShoppingList', (items: string[]) => {
          this.shoppingList = items;
        });

      this.signalrService
        .getHubConnection()
        .on('ShoppingListCreated', (createdId: string) => {
          this.shoppingListId = createdId;
        });

      this.signalrService
        .getHubConnection()
        .on('JoinShoppingList', (shoppingListId: string, items: string[]) => {
